...
%macro core_cas_initiate_session(cas_host =
   ...
   %if not(%sysevalf(%superq(cas_host) =, boolean) and %sysevalf(%superq(cas_port) =, boolean)) %then
      options cashost="&cas_host." casport=&cas_port. &cas_options. %str(;);
   %else %if not(%sysevalf(%superq(cas_options) =, boolean)) %then
      options &cas_options. %str(;);
   ...
   %if not(%sysfunc(sessfound(&cas_session_name.))) %then %do;
      ...
      %if not(%sysevalf(%superq(cas_session_options) =, boolean)) %then
         /* DAM - Add specific session options*/
         sessopts=(&cas_session_options.)%str(;);
         ;
      %if not(%sysfunc(sessfound(&cas_session_name.))) %then %do;
